...
| File | Change |
|---|---|
| `autobots-devtools-shared-lib/tests/unit/test_agent_config_utils.py` | Add tests for `load_template` and `load_render_manifest` against existing `configs/bro` fixture (extended with `templates/` + `dynadoc.yaml`) |
...
- Create fixture: `autobots-devtools-shared-lib/configs/bro/templates/sample.md.j2`
- Create fixture: `autobots-devtools-shared-lib/configs/bro/dynadoc.yaml`

- [ ] **Step 1: Add fixture files**

Create `configs/bro/templates/sample.md.j2` with:
```jinja
# Hello {{ name }}
```

Create `configs/bro/dynadoc.yaml` with:
```yaml
documents:
  smoke:
    json: smoke.json
...
git add src/autobots_devtools_shared_lib/dynagent/agents/agent_config_utils.py \
        ...
        configs/bro/templates/sample.md.j2 \
        configs/bro/dynadoc.yaml
...
- Modify: `autobots-devtools-shared-lib/tests/unit/dynadoc/test_engine.py`
- Create fixture: `autobots-devtools-shared-lib/configs/bro/templates/d.md.j2`
- Modify fixture: `autobots-devtools-shared-lib/configs/bro/dynadoc.yaml`

- [ ] **Step 1: Extend bro fixture for end-to-end test**

**Important:** Do **not** replace `configs/bro/dynadoc.yaml` — Task 2's tests assert against the existing `smoke` entry pointing at `sample.md.j2`. Instead, *add* a second document entry.

Update `configs/bro/dynadoc.yaml` to:
...
Create `configs/bro/templates/d.md.j2`:

```jinja
hello {{ who }}
```

(`configs/bro/templates/sample.md.j2` from Task 2 stays — it's referenced by other tests.)
