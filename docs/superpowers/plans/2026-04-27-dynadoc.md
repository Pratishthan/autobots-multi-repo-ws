...
def _render_with_template(
    ...
    try:
        ...
        # Syntax errors are author bugs — surface in both modes.
        raise UndefinedVariableError(
...
Replace `tests/configs/bro/dynadoc.yaml` with:
...
def test_render_document_end_to_end(monkeypatch):
    ...
    result = render_document("smoke", load_json=load_json, strict=True)
...
In `autobots-devtools-shared-lib/pyproject.toml`, bump version from `0.6.1` → `0.7.0` (new feature, additive). Update the workspace-level CLAUDE.md table if the version is referenced there:
